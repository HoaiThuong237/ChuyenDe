...
const UpdateRecipe = () => {
    ...
    const handleSaveRecipe = async () => {
        if (title === '' || description === '' || steps.length === 0 || ingredients.length === 0) {
            alert('Vui lòng nhập đầy đủ thông tin công thức.');
            return;
        } else {
            try {
        ...
        <Stack spacing={2}>
            <Stack direction={"row"} spacing={2}>
                ...
                <Button variant="contained" 
                        ...
                        startIcon={<CloseIcon />}
                        onClick={() => {
                            const confirmcancle = window.confirm("Bạn có chắc muốn hủy? Mọi thay đổi sẽ bị mất.")
                            if (confirmcancle) {
                                navigate("/congthuc")
                            }
                        }}
